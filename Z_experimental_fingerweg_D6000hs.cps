/**
  ...
  $Revision: 38621 $
  $Date: 2015-03-03 19:48:52 +0100 (ti, 03 mar 2015) $
...
properties = {
  ...
  preloadTool: false, // preloads next tool on tool change if any //Toni:changed to false
  showSequenceNumbers: true, // show sequence numbers
  sequenceNumberStart: 10, // first sequence number
  sequenceNumberIncrement: 5, // increment for sequence numbers
  maximumSpindleSpeed: 100 * 48, // specifies the maximum spindle speed
...
var pitchOutput = createVariable({prefix:"K", force:true}, pitchFormat); //Toni
var sOutput = createVariable({prefix:"S", force:true}, rpmFormat);
var dOutput = createVariable({}, dFormat);
var pOutput = createVariable({prefix:"P", force:true}, rpmFormat); //Toni
...
var gSpindleModeModal = createModal({}, gFormat); // modal group 5 // G96-97 //toni
...
function onOpen() {
  ...
  // absolute coordinates, feed per min, XZ-plane, incremental arc center mode, and radius mode
  writeComment( "absolute coordinates, feed per min, XZ-plane, incremental arc center mode, radius mode, cutter compensation off");
//original  writeBlock(gAbsIncModal.format(90), gFeedModeModal.format(94), gPlaneModal.format(17), gFormat.format(91.1));
  writeBlock(gAbsIncModal.format(90), 
  gFeedModeModal.format(94), 
  gPlaneModal.format(18), 
  gFormat.format(91.1),
  gUnitModal.format(8),
  gFormat.format(40)); //toni


  switch (unit) {
  case IN:
    error(localize("inch mode not support")); //toni, because not needed and for security 
    writeBlock(gUnitModal.format(20));
    break;
  case MM:
    writeComment("unit=mm");
...
function forceXYZ() {
  xOutput.reset();
//  yOutput.reset(); //Toni, TODO extra funktion force(XZ);
  zOutput.reset();
}
...
//      writeComment("retract to safe plane");  
//      writeBlock(gAbsIncModal.format(90), gFormat.format(53), gMotionModal.format(0), "X" + xyzFormat.format(0)); // retract //Toni
   }
    zOutput.reset();
  }
  ...
  if (insertToolCall) {
    ...
    retracted = true;
    //onCommand(COMMAND_COOLANT_OFF); //Toni
    ...
    writeComment("Tool Length Offset");    
    writeBlock(gFormat.format(43));//Toni
    if (tool.comment) {
...
//Toni, commentet out, because we have no coolant
//  {
...
//  }
  ...
  if (insertToolCall || retracted || (!isFirstSection() && getPreviousSection().isMultiAxis())) {
    ...
    gMotionModal.reset();
    writeBlock(gPlaneModal.format(18)); //toni, original 17
    
    if (!machineConfiguration.isHeadConfiguration()) {
      writeBlock(
        gAbsIncModal.format(90),
//        gMotionModal.format(0), xOutput.format(initialPosition.x), yOutput.format(initialPosition.y) //original
        gMotionModal.format(0), xOutput.format(initialPosition.x) //Toni
      );
      //writeBlock(gMotionModal.format(0), gFormat.format(43), zOutput.format(initialPosition.z), hFormat.format(lengthOffset)) //original;
    } else {
      writeBlock(
        gAbsIncModal.format(90),	
        gMotionModal.format(0)//,
        //gFormat.format(43), xOutput.format(initialPosition.x),	//Toni
        //yOutput.format(initialPosition.y),				//Toni
        //zOutput.format(initialPosition.z), hFormat.format(lengthOffset) //Toni
      );
    }
...
function onCycle() {
  //writeBlock(gPlaneModal.format(17)); //Toni
  writeComment("warning: onCycle!!!");
}
...
//    writeComment("getCommonCycle");
}

function onCyclePoint(x, y, z) {
  writeComment("onCyclePoint");
  switch (cycleType) {
  case "tapping":
  case "left-tapping":
  case "right-tapping":
...
//  writeComment("on rapid");
  ...
  if (x || y || z) {
    ...
    //writeBlock(gMotionModal.format(0), x, y, z); //original
    writeBlock(gMotionModal.format(0), x, z); //Toni
    forceFeed();
  }
}
...
//Toni begin
  if (isSpeedFeedSynchronizationActive()) {
    ...
    writeBlock(gMotionModal.format(33), x, y, z, pitchOutput.format(threadPitch));
    return;
  }
...
//Toni end
  ...
  if (x || y || z) {
    if (pendingRadiusCompensation >= 0) {
      writeComment("radius compensation called");
...
function onSectionEnd() {
  ...
  //writeBlock(gPlaneModal.format(17)); //Toni senseless and wrong
