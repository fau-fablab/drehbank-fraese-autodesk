...
properties = {
  ...
  preloadTool: false, // preloads next tool on tool change if any // Toni: changed from true to false
...
function onOpen() {
  ...
  //cancel cutter compensation radius
  writeBlock(gUnitModal.format(40));  //Toni
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane) {
    ...
    retracted = true;
    //writeBlock(gFormat.format(28)); // retract  //Toni: uncommented
    writeBlock(gMotionModal.format(0), gMotionModal.format(53), zOutput.format(-0.001)); //Toni
