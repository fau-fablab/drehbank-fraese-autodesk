...
function onOpen() {
  ...
  if (true) { // note: setup your machine here: aktiviere A-Achse (und nur diese)
    var aAxis = createAxis({coordinate:0, table:true, axis:[1, 0, 0], range:[-360, 360], preference:0, cyclic:true});
//    var bAxis = createAxis({coordinate:1, table:true, axis:[0, 1, 0], range:[-360, 360], preference:0, cyclic:true});
//    var cAxis = createAxis({coordinate:2, table:true, axis:[0, 0, 1], range:[-360, 360], preference:0, cyclic:true});
    machineConfiguration = new MachineConfiguration(aAxis);
 
    setMachineConfiguration(machineConfiguration);
    optimizeMachineAngles2(0); // TCP mode: abgeschaltet, Maschinensteuerung unterstützt das nicht.
  }
